...
static inline s32 __ipcSendRequest(void) {
    ...
    if (waiting != 0) {
        ...
        IPCWriteReg(IPC_PPCMSG, (u32)OSCachedToPhysical(req));
        __responses.front = (__responses.front + 1) % IPC_QUEUE_CAPACITY;
        __responses.sent++;
        __mailboxAck--;
        IPCWriteReg(IPC_PPCCTRL, (IPCReadReg(IPC_PPCCTRL) & 0x30) | 1);
    }
...
#pragma unused(intr)
    ...
    reg = IPCReadReg(IPC_ARMMSG);
    if (reg != 0) {
        req = (IPCRequestEx*)OSPhysicalToCached(reg);

        IPCWriteReg(IPC_PPCCTRL, (IPCReadReg(IPC_PPCCTRL) & 0x30) | 4);
        ...
        IPCWriteReg(IPC_PPCCTRL, (IPCReadReg(IPC_PPCCTRL) & 0x30) | 8);
...
#pragma unused(ctx)
    ...
    IPCWriteReg(IPC_PPCCTRL, (IPCReadReg(IPC_PPCCTRL) & 0x30) | 2);
    ACRWriteReg(ACR_PPCIRQFLAG, 0x40000000);
    ...
    if (__mailboxAck > 0) {
        if (__relnchFl) {
            ...
            IPCWriteReg(IPC_PPCCTRL,
                        (IPCReadReg(IPC_PPCCTRL) & 0x30) | 8);
        }
        __ipcSendRequest();
    }
}
...
static void IPCInterruptHandler(s16 intr, OSContext* ctx) {
    if ((IPCReadReg(IPC_PPCCTRL) & 0x14) == 0x14) {
        IpcReplyHandler(intr, ctx);
    }

    if ((IPCReadReg(IPC_PPCCTRL) & 0x22) == 0x22) {
