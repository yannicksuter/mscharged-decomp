...
static void PADReceiveCheckCallback(s32 chan, u32 type)
{
    ...
    if (!(error & (SI_ERROR_UNDER_RUN | SI_ERROR_OVER_RUN | SI_ERROR_NO_RESPONSE | SI_ERROR_COLLISION)) && (type & SI_GC_WIRELESS) && (type & SI_WIRELESS_FIX_ID) && (type & SI_WIRELESS_RECEIVED) && !(type & SI_WIRELESS_IR) && !(type & SI_WIRELESS_CONT_MASK) && !(type & SI_WIRELESS_LITE))
    {
...
// The retail image links no PAD version banner and no PADInit, so the
// registration this file would normally perform is left out rather than
// reconstructed from an unknown build string.
BOOL PADInit(void)
{
    s32 chan;
...
static void SPEC2_MakeStatus(s32 chan, PADStatus* status, u32 data[2])
{
    ...
    if (__abs(SQ(status->stickX) - SQ(PrevStatus[chan].stickX)) >= 9 || __abs(SQ(status->stickY) - SQ(PrevStatus[chan].stickY)) >= 9 || __abs(SQ(status->substickX) - SQ(PrevStatus[chan].substickX)) >= 9 || __abs(SQ(status->substickY) - SQ(PrevStatus[chan].substickY)) >= 9 || __abs(SQ(status->triggerLeft) - SQ(PrevStatus[chan].triggerLeft)) >= 9 || __abs(SQ(status->triggerRight) - SQ(PrevStatus[chan].triggerRight)) >= 9 || status->button != PrevStatus[chan].button)
    {
        __VIResetSIIdle();
    }
