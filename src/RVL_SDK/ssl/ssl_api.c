...
enum
{
    ...
    SSL_IOCTL_SET_CLIENT_CERT_DEFAULT,
    SSL_IOCTL_SET_ROOT_CA = 10,
    SSL_IOCTL_SET_ROOT_CA_DEFAULT,
};
...
SSLId SSLNew(u32 i_verifyOption, const char* i_serverName)
{
    ...
    {
        IPCIOVector vec[MAX_IOS_VECS] ALIGN(32);
        char server_name[SSL_SERVER_NAME_LENGTH];
        u32 name_len = SSL_strnlen(i_serverName, sizeof(server_name));
        SSLId ssl_id[SSL_MAX] ALIGN(32);
        u32 verify_option[SSL_MAX] ALIGN(32);
        s32 err;
...
SSLResult SSLConnect(SSLId i_sslId, SSLSocket i_socket)
{
    ...
    else
    {
        IPCIOVector vec[MAX_IOS_VECS] ALIGN(32);
        SSLId ssl_id[SSL_MAX] ALIGN(32);
        SSLSocket socket[SSL_MAX] ALIGN(32);
        SSLResult result[SSL_MAX] ALIGN(32);
        s32 err;
...
static char l_cert_data[SSL_CLIENT_CERT_LENGTH] ALIGN(32);
...
static char l_private_key_data[SSL_PRIVATE_KEY_LENGTH] ALIGN(32);

SSLResult SSLSetClientCert(SSLId i_sslId, const char* i_clientCertData, u32 i_clientCertSize,
    const char* i_privateKeyData, u32 i_privateKeySize)
{
    s32 fd = OPEN_SSL_IOS();
...
SSLResult SSLSetClientCertDefault(SSLId i_sslId)
{
    s32 fd = OPEN_SSL_IOS();
    ...
    else
    {
        ...
        *ssl_id = i_sslId;
        ...
        err = IOS_Ioctlv(fd, SSL_IOCTL_SET_CLIENT_CERT_DEFAULT, 1, 1, vec);
        IOS_Close(fd);
...
static char l_root_ca[SSL_ROOT_CA_LENGTH] ALIGN(32);
...
SSLResult SSLSetRootCADefault(SSLId i_sslId)
{
    s32 fd = OPEN_SSL_IOS();
    ...
    else
    {
        ...
        *ssl_id = i_sslId;
        ...
        err = IOS_Ioctlv(fd, SSL_IOCTL_SET_ROOT_CA_DEFAULT, 1, 1, vec);
        IOS_Close(fd);
