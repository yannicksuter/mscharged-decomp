...
GPResult gpGetBuddyStatus(GPConnection* connection, int index,
  ...
  if (buddyStatus->statusString)
    strzcpy(status->statusString, buddyStatus->statusString,
            GP_STATUS_STRING_LEN);
  else
    status->statusString[0] = '\0';
  if (buddyStatus->locationString)
    strzcpy(status->locationString, buddyStatus->locationString,
...
GPResult gpSendBuddyUTMA(GPConnection* connection, GPProfile profile,
  ...
  return gpiSendBuddyMessage(connection, profile, GPI_BM_UTM, message, sendOption);
}
...
GPResult gpSendFilesA(GPConnection* connection, GPTransfer* transfer,
  ...
  // Ping the receiver.
  /////////////////////
  result = gpiSendBuddyMessage(connection, profile, GPI_BM_PING, "1", 0);
  if (result != GP_NO_ERROR) {
    gpiFreeTransfer(connection, pTransfer);
    return result;
  }
