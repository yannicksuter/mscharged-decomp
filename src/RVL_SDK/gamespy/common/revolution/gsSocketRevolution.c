...
struct hostent* gethostbyname(const char* name) {
  ...
  for (i = 0; i < DNSCACHESIZE / 2; i++) {
    ...
    if ((data != NULL) && (strcmp(data->name, name) == 0)) {
      gsDebugFormat(GSIDebugCat_Common, GSIDebugType_Network,
                    GSIDebugLevel_Notice, "[DNS cache] hit!! %s\n", name);
  ...
  if (dnscache[hash] != NULL) {
    ...
    gsDebugFormat(GSIDebugCat_Common, GSIDebugType_Network,
                  GSIDebugLevel_Notice, "[DNS cache] trying to rehash\n");
  ...
  GS_ASSERT(dnscache[hash] != NULL);

  {
    GS_ASSERT(hostentry->h_addrtype == AF_INET);
    ...
    dnscache[hash]->entry.h_addr_list =
        (u8**)gsimalloc(sizeof(u8*) * (i + 1));

    for (i = 0; hostentry->h_addr_list[i] != NULL; i++) {
      dnscache[hash]->entry.h_addr_list[i] =
          (u8*)gsimalloc(hostentry->h_length);

      memcpy(dnscache[hash]->entry.h_addr_list[i], hostentry->h_addr_list[i],
             hostentry->h_length);
    }
  ...
  GS_ASSERT(dnscache[hash]->name != NULL);
