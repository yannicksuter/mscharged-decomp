...
GHTTPBool ghiSendBufferedData(struct GHIConnection* connection) {
  ...
  do {
    ...
    rcode = ghiDoSend(connection,
                      connection->sendBuffer.data + connection->sendBuffer.pos,
...
GHTTPBool ghiReadDataFromBufferFixed(GHIBuffer* bufferIn, char bufferOut[],
  ...
  // Verify that the buffer holds enough data.
  ////////////////////////////////////////////
  if (bufferIn->len < len)
    return GHTTPFalse;
  ...
  //////////////////////////////////////////
  memcpy(bufferOut, bufferIn->data + bufferIn->pos, len);
  bufferIn->pos += len;
  return GHTTPTrue;
}
