...
GT2Bool gti2SendAppUnreliable(GT2Connection connection, const GT2Byte* message, int len)
{
    ...
    if ((len < GTI2_MAGIC_STRING_LEN) || (memcmp(message + connection->socket->protocolOffset, GTI2_MAGIC_STRING, GTI2_MAGIC_STRING_LEN) != 0))
    {
    ...
    gti2BufferWriteData(&connection->outgoingBuffer, message + connection->socket->protocolOffset, (int)(len - connection->socket->protocolOffset));
