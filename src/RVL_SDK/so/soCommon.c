...
int SOStartupEx(int timeout) {
  ...
  limitTime = 0;
  if (timeout != 0)
    limitTime = __OSGetSystemTime() + ((timeout) * ((OS_BUS_CLOCK_SPEED / 4) / 1000));
    ...
    if (soWork.rmState > SO_INTERNAL_RM_STATE_CLOSED) {
      ...
      while (true) {
        ...
        if (resultNCD != NCD_RESULT_INPROGRESS &&
          ...
          break;
        OSSleepTicks((((s64)100) * ((OS_BUS_CLOCK_SPEED / 4) / 1000)));
  ...
  if (result == 0) {
    ...
    if (limitTime != 0 && dhcpTimeOutTicks <= 0) {
      ...
      result = SOiWaitForDHCPEx(
          (int)((dhcpTimeOutTicks) / ((OS_BUS_CLOCK_SPEED / 4) / 1000)));
    }
...
int SOiWaitForDHCPEx(int timeout) {
  ...
  while (true) {
    OSSleepTicks((((s64)10) * ((OS_BUS_CLOCK_SPEED / 4) / 1000)));
