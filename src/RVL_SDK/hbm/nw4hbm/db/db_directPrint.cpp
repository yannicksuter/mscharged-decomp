...
static inline int StrLineWidth_(char const* str) {
    ...
    NW4HBMAssertPointerNonnull_Line(str, 306);
...
void DirectPrint_ChangeXfb(void* framebuf) { sFrameBufferInfo.frameMemory = static_cast<byte_t*>(framebuf); }

void DirectPrint_StoreCache(void) { DCStoreRange(sFrameBufferInfo.frameMemory, sFrameBufferInfo.frameSize); }
...
void detail::DirectPrint_DrawStringToXfb(int posh, int posv, char const* format, va_list vargs, bool turnOver,
                                         bool backErase) {
    char string[256];
...
static char const* DrawStringLineToXfb_(int posh, int posv, char const* str, int width) {
    ...
    NW4HBMAssertPointerNonnull_Line(str, 745);
...
static void DrawCharToXfb_(int posh, int posv, int code) {
    ...
    for (int cntv = 0; cntv < 7; cntv++) {
        ...
        if (wH == 1) {
            ...
            fontBits = (twiceBit[(fontBits >> 26) & 3] | twiceBit[(fontBits >> 28) & 3] << 4 |
