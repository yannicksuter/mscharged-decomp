...
bool EqualsPaneName(const char* name1, const char* name2) { return strncmp(name1, name2, 16) == 0; }

// sizeof(Material::mName) == 20
bool EqualsMaterialName(const char* name1, const char* name2) { return strncmp(name1, name2, 20) == 0; }
...
void InitGXTexObjFromTPL(GXTexObj* to, TPLPalette* pal, u32 id) {
    ...
    GXBool mipMap = tdp->textureHeader->minLOD != tdp->textureHeader->maxLOD ? (GXBool)1 : (GXBool)0;
    ...
    if (tdp->CLUTHeader) {
        ...
        GXInitTexObjCI(to, tdp->textureHeader->data, tdp->textureHeader->width, tdp->textureHeader->height,
                       static_cast<GXCITexFmt>(tdp->textureHeader->format), tdp->textureHeader->wrapS, tdp->textureHeader->wrapT,
        ...
        GXInitTexObj(to, tdp->textureHeader->data, tdp->textureHeader->width, tdp->textureHeader->height,
                     static_cast<GXTexFmt>(tdp->textureHeader->format), tdp->textureHeader->wrapS, tdp->textureHeader->wrapT,
