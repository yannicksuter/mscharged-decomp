...
void* FrameHeap::Alloc(u32 size, FreeCallback callback, void* callbackArg)
{
    ...
    NW4HBMAssertMessage_Line(NW4HBM_IS_ALIGNED_((u32)pBlock + BLOCK_BUFFER_SIZE, HEAP_ALIGN), 174, "FrameHeap::Alloc: Internal Error");
...
{
    NW4HBMAssertHeaderClampedLValue(level, 0, mSectionList.GetSize());
    ...
    while (level < static_cast<int>(mSectionList.GetSize()))
    {
    ...
    NW4HBMAssertMessage(MEMFreeByStateToFrmHeap(mHandle, level),
