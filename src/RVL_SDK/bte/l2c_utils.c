...
tL2C_LCB *l2cu_find_lcb_by_bd_addr(BD_ADDR p_bd_addr)
{
	...
	{
		if (p_lcb->in_use
		    && memcmp(p_lcb->remote_bd_addr, p_bd_addr, BD_ADDR_LEN) == 0)
		{
			return p_lcb;
		}
	}
                               ...
                               UINT16 p1, UINT16 p2)
{
	...
	if ((p_buf = l2cu_build_header(p_lcb->handle,
	                               (UINT16)(L2CAP_CMD_REJECT_LEN + param_len),
...
void l2cu_send_peer_config_req(tL2C_CCB *p_ccb, tL2CAP_CFG_INFO *p_cfg)
{
	...
	if ((p_buf = l2cu_build_header(p_ccb->p_lcb->handle,
	                               L2CAP_CONFIG_REQ_LEN + cfg_len,
                               ...
                               UINT16 rej_len)
{
	...
	UINT16_TO_STREAM(p, L2CAP_PKT_START << L2CAP_PKT_TYPE_SHIFT
	                        | p_ccb->p_lcb->handle);
...
void l2cu_send_peer_echo_req(tL2C_LCB *p_lcb, UINT8 *p_data, UINT16 data_len)
{
	...
	if ((p_buf = l2cu_build_header(p_lcb->handle, L2CAP_ECHO_REQ_LEN + data_len,
                             ...
                             UINT16 data_len)
{
	...
	maxlen = GKI_BUF2_SIZE > btu_cb.hcit_acl_pkt_size
	           ? btu_cb.hcit_acl_data_size
	           : GKI_BUF2_SIZE;
	maxlen -= (UINT16)(HCI_DATA_PREAMBLE_SIZE + L2CAP_PKT_OVERHEAD
	                   + L2CAP_CMD_OVERHEAD + L2CAP_ECHO_RSP_LEN);

	if (data_len > maxlen)
		data_len = 0;

	if ((p_buf = l2cu_build_header(p_lcb->handle, L2CAP_ECHO_RSP_LEN + data_len,
...
void l2cu_send_peer_info_req(tL2C_LCB *p_lcb, UINT16 info_type)
{
	...
	if ((p_buf = l2cu_build_header(p_lcb->handle, 2, L2CAP_CMD_INFO_REQ,
	                               ++p_lcb->id))
	    == NULL)
	{
		L2CAP_TRACE(WARNING, "L2CAP - no buffer for conn_req");
...
void l2cu_send_peer_info_rsp(tL2C_LCB *p_lcb, UINT8 remote_id, UINT16 info_type)
{
	...
	if ((p_buf = l2cu_build_header(p_lcb->handle, len, L2CAP_CMD_INFO_RSP,
	                               remote_id))
	    == NULL)
	{
		L2CAP_TRACE(WARNING, "L2CAP - no buffer for conn_req");
...
tL2C_CCB *l2cu_allocate_ccb(tL2C_LCB *p_lcb)
{
	...
	p_ccb->in_flow.delay_variation = p_ccb->out_flow.delay_variation =
		0xffffffff;
	...
	if (p_cfg->flush_to_present)
	{
		...
		if (p_cfg->flush_to < p_lcb->link_flush_tout)
		{
			...
			if (p_cfg->flush_to <= 0x4ff)
			{
				...
				if (p_buf)
				{
					btsnd_hcic_write_auto_flush_tout(p_buf, p_lcb->handle,
					                                 hci_flush_to);
				}
			}
		}
...
BOOLEAN l2cu_create_conn(tL2C_LCB *p_lcb)
{
	...
	/* explicitly post-increment */
	for (xx = 0, p_lcb_cur = l2cb.lcb_pool; xx < MAX_L2CAP_LINKS;
	     xx++, p_lcb_cur++)
	{
		if (p_lcb_cur == p_lcb)
			continue;
		...
		{
			is_sco_active =
				btm_is_sco_active_by_bdaddr(p_lcb_cur->remote_bd_addr);
			...
			if (HCI_SWITCH_SUPPORTED(btm_cb.devcb.local_features))
			{
				...
				BTM_SwitchRole(p_lcb_cur->remote_bd_addr, HCI_ROLE_MASTER,
				               NULL);

				btu_start_timer(&p_lcb->timer_entry, BTU_TTYPE_L2CAP_LINK,
				                L2CAP_LINK_ROLE_SWITCH_TOUT);
...
BOOLEAN l2cu_create_conn_after_switch(tL2C_LCB *p_lcb)
{
	...
	if (!btsnd_hcic_create_conn(p_lcb->remote_bd_addr,
	                            ...
	                            page_scan_rep_mode, page_scan_mode,
                               ...
                               UINT16 data_len)
{
	...
	if (data_len < 4 || p_data[0] != 'R' || p_data[1] != 'Q'
	    || p_data[data_len - 2] != 'r' || p_data[data_len - 1] != 'q'
                            ...
                            UINT16 data_len)
{
	...
	if (data_len <= 4 || p_data[0] != 'R' || p_data[1] != 'S'
	    || p_data[data_len - 2] != 'r' || p_data[data_len - 1] != 's'
