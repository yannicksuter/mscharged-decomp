...
void read_kpad_acc(KPADInsideStatus* kp, KPADUnifiedWpadStatus* uwp) {
    ...
    if (uwp->u.fs.err != WPAD_ERR_OK || uwp->u.fs.dev != WPAD_DEV_FREESTYLE ||
...
void check_kobj_outside_frame(KPADInsideStatus* kp, KPADObject* kobj_t) {
    KPADObject* kobj_p = &kobj_t[WPAD_MAX_DPD_OBJECTS - 1];
...
void read_kpad_stick(KPADInsideStatus* kp, KPADUnifiedWpadStatus* uwp) {
    ...
    if (uwp->u.fs.dev == WPAD_DEV_FREESTYLE &&
        (uwp->fmt == WPAD_FMT_FS_BTN || uwp->fmt == WPAD_FMT_FS_BTN_ACC || uwp->fmt == WPAD_FMT_FS_BTN_ACC_DPD)) {
        if (kp->exResetReq) {
            kp->exResetReq = FALSE;
        ...
        clampStickFuncp(&ep->fs.stick, uwp->u.fs.fsStickX, uwp->u.fs.fsStickY, kp_fs_fstick_min, kp_fs_fstick_max);
               ...
               (uwp->fmt == WPAD_FMT_CLASSIC_BTN || uwp->fmt == WPAD_FMT_CLASSIC_BTN_ACC || uwp->fmt == WPAD_FMT_CLASSIC_BTN_ACC_DPD)) {
        if (kp->exResetReq) {
            kp->exResetReq = FALSE;
...
void read_kpad_button(KPADInsideStatus* kp, u32 dev_type, u32 count, u32 core, u32 fs, u32 cl) {
    ...
    sp->hold = (core & (KPAD_BUTTON_MASK & ~(WPAD_BUTTON_FS_Z | WPAD_BUTTON_FS_C))) | (fs & (WPAD_BUTTON_FS_Z | WPAD_BUTTON_FS_C));
...
s32 KPADRead(KPADChannel chan, KPADStatus samplingBufs[], u32 length) {
    ...
    if (wpad_err == WPAD_ERR_NO_CONTROLLER && kp->dpd_ctrl_callback && kp->dpdPreCallbackDone && !kp->dpdPostCallbackDone) {
        KPADiControlDpdCallback(chan, WPAD_ERR_OK);
    }
    ...
    if (kp->bufCount && samplingBufs && length) {
        ...
        do {
            ...
            if (kp->status.dev_type != uwp->u.core.dev) {
                if (uwp->u.core.err == WPAD_ERR_OK || uwp->u.core.err == WPAD_ERR_BUSY || uwp->u.core.err == WPAD_ERR_NO_CONTROLLER) {
...
static void KPADiSamplingCallback(s32 chan) {
    ...
    WPADRead(chan, (WPADStatus*)&uwp->u);
