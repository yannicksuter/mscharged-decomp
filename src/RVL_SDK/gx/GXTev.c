...
void GXSetTevSwapModeTable(GXTevSwapSel table, GXTevColorChan red, GXTevColorChan green, GXTevColorChan blue,
    ...
    reg = &gxdt->tevKsel[table << 1];
    GX_SET_REG(*reg, red, 30, 31);
    GX_SET_REG(*reg, green, 28, 29);

    GX_BP_LOAD_REG(*reg);

    reg = &gxdt->tevKsel[(table << 1) + 1];
...
void GXSetTevOrder(GXTevStageID stage, GXTexCoordID coord, GXTexMapID map, GXChannelID color) {
    ...
    reg = &gxdt->tref[stage / 2];
    gxdt->texmapId[stage] = map;
    ...
    if (coord >= GX_MAX_TEXCOORD) {
        ...
        gxdt->tevTcEnab = gxdt->tevTcEnab & ~(1 << stage);
    } else {
        tempCoord = coord;
        gxdt->tevTcEnab = gxdt->tevTcEnab | (1 << stage);
    }
...
void GXSetNumTevStages(u8 count) {
    GX_SET_REG(gxdt->genMode, count - 1, 18, 21);

    gxdt->gxDirtyFlags |= 0x4;
}
