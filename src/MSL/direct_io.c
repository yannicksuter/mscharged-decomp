...
size_t __fread(void* buffer, size_t size, size_t count, FILE* stream)
{
    ...
    if (!bytes_to_go || stream->file_state.error || stream->file_mode.file_kind == file_closed)
        return 0;
    ...
    if (stream->file_state.io_state == neutral)
    {
        if (stream->file_mode.io_mode & __read)
        {
            stream->file_state.io_state = reading;
            stream->buffer_length = 0;
        }
    }

    if (stream->file_state.io_state < reading)
    {
        set_error(stream);
        return 0;
    }
    ...
    if (bytes_to_go && stream->file_state.io_state >= rereading)
    {
        do
        {
            if (fwide(stream, 0) == 1)
            {
                bytes_read += 2;
                bytes_to_go -= 2;
                *(wchar_t*)read_ptr = stream->ungetc_wide_buffer[stream->file_state.io_state - rereading];
                read_ptr += 2;
            }
            else
            {
                bytes_read += 1;
                bytes_to_go -= 1;
                *read_ptr = stream->ungetc_buffer[stream->file_state.io_state - rereading];
                read_ptr += 1;
            }
            ...
            if (!bytes_to_go)
                break;
        } while (stream->file_state.io_state >= rereading);
    ...
    if (bytes_to_go)
    {
        if (stream->buffer_length || always_buffer)
        {
            do
            {
                ...
                {
                    ioresult = __load_buffer(stream, NULL, __align_buffer);
                    if (ioresult)
                    {
                        ...
                        {
                            stream->file_state.io_state = neutral;
    ...
    if (bytes_to_go && !always_buffer)
    {
        ...
        ioresult = __load_buffer(stream, &num_bytes, __dont_align_buffer);
        if (ioresult)
        {
            if (ioresult == __io_error)
            {
...
size_t __fwrite(const void* buffer, size_t size, size_t count, FILE* stream)
{
    ...
    if (stream->file_mode.file_kind == file_console)
        __stdio_atexit();
