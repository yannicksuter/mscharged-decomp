...
typedef struct
{
	tHCI_HANDLE				hci_handle;			// 0x000
	UINT16					pkt_types_mask;		// 0x002
	UINT16					restore_pkt_types;	// 0x004
	UINT16					clock_offset;		// 0x006
	BD_ADDR					remote_addr;		// 0x008
	DEV_CLASS				remote_dc;			// 0x00e
	BD_NAME					remote_name;		// 0x011
	/* 1 byte padding */
	UINT16					manufacturer;		// 0x10a
	UINT16					lmp_subversion;		// 0x10c
	UINT16					link_super_tout;	// 0x10e
	BD_FEATURES				features;			// 0x110
	UINT8					lmp_version;		// 0x118
	BOOLEAN					in_use;				// 0x119
	tBTM_ROLE				link_role;			// 0x11a
	tBTM_ACL_SWKEY_STATE	switch_role_state;	// 0x11b
} tACL_CONN; // size 0x11c

typedef struct
{
	tBTM_DEV_STATUS_CB		*p_dev_status_cb;			// 0x00
	tBTM_VS_EVT_CB			*p_vend_spec_cb;			// 0x04
	tBTM_CMPL_CB			*p_stored_link_key_cmpl_cb;	// 0x08
	TIMER_LIST_ENT			reset_timer;				// 0x0c
	tBTM_CMPL_CB			*p_reset_cmpl_cb;			// 0x24
	TIMER_LIST_ENT			rln_timer;					// 0x28
	tBTM_CMPL_CB			*p_rln_cmpl_cb;				// 0x40
	TIMER_LIST_ENT			rlinkp_timer;				// 0x44
	tBTM_CMPL_CB			*p_rlinkp_cmpl_cb;			// 0x5c
	TIMER_LIST_ENT			rssi_timer;					// 0x60
	tBTM_CMPL_CB			*p_rssi_cmpl_cb;			// 0x78
	TIMER_LIST_ENT			lnk_quality_timer;			// 0x7c
	...
	BD_ADDR					local_addr;					// 0xc8
	tBTM_VERSION_INFO		local_version;				// 0xce
	BD_FEATURES				local_features;				// 0xd8
	DEV_CLASS				dev_class;					// 0xe0
	/* 1 byte padding */
	UINT16					page_timeout;				// 0xe4
	tBTM_DEV_STATE			state;						// 0xe6
	UINT8					retry_count;				// 0xe7
	UINT8					vsc_busy;					// 0xe8
	/* 3 bytes padding */
...
typedef struct
{
	tBTM_CMPL_CB				*p_remname_cmpl_cb;				// 0x000
	TIMER_LIST_ENT				rmt_name_timer_ent;				// 0x004
	tBTM_DISC_MODE				discoverable_mode;				// 0x01c
	tBTM_CONN_MODE				connectable_mode;				// 0x01e
	UINT16						page_scan_window;				// 0x020
	UINT16						page_scan_period;				// 0x022
	UINT16						inq_scan_window;				// 0x024
	UINT16						inq_scan_period;				// 0x026
	tBTM_INQ_SCAN_TYPE			inq_scan_type;					// 0x028
	tBTM_INQ_SCAN_TYPE			page_scan_type;					// 0x02a
	BD_ADDR						remname_bda;					// 0x02c
	tBTM_RMT_NAME_ACTIVE_FLAGS	remname_active;					// 0x032
	/* 1 byte padding */
	tBTM_CMPL_CB				*p_inq_cmpl_cb;					// 0x034
	tBTM_INQ_RESULTS_CB			*p_inq_results_cb;				// 0x038
	tBTM_CMPL_CB				*p_inqfilter_cmpl_cb;			// 0x03c
	tBTM_INQ_DB_CHANGE_CB		*p_inq_change_cb;				// 0x040
	UINT32						inq_counter;					// 0x044
	TIMER_LIST_ENT				inq_timer_ent;					// 0x048
	tINQ_BDADDR					*p_bd_db;						// 0x060
	UINT16						num_bd_entries;					// 0x064
	UINT16						max_bd_entries;					// 0x066
	tINQ_DB_ENT					inq_db[BTM_INQ_DB_SIZE];		// 0x068
	tBTM_INQ_PARMS				inqparms;						// 0x1b8
	tBTM_INQUIRY_CMPL			inq_cmpl_info;					// 0x1c2
	UINT16						per_min_delay;					// 0x1c4
	UINT16						per_max_delay;					// 0x1c6
	BOOLEAN						inqfilt_active;					// 0x1c8
	UINT8						pending_filt_complete_event;	// 0x1c9
	tBTM_INQ_FILT_COND_TYPE		inqfilt_type;					// 0x1ca
	tBTM_INQ_STATE				state;							// 0x1cb
	tBTM_INQ_ACTIVE_STATUS		inq_active;						// 0x1cc
	/* 3 bytes padding */
} tBTM_INQUIRY_VAR_ST; // size 0x1d0

typedef struct
{
	tBTM_ESCO_CBACK		*p_esco_cback;	// 0x00
	tBTM_ESCO_PARAMS	setup;			// 0x04
	tBTM_ESCO_DATA		data;			// 0x14
	tHCI_STATUS			hci_status;		// 0x22
	/* 1 byte padding */
} tBTM_ESCO_INFO; // size 0x24

typedef struct
{
	tBTM_SCO_CB		*p_conn_cb;		// 0x00
	tBTM_SCO_CB		*p_disc_cb;		// 0x04
	UINT16			state;			// 0x08
	tHCI_HANDLE		hci_handle;		// 0x0a
	BOOLEAN			is_orig;		// 0x0c
	BOOLEAN			rem_bd_known;	// 0x0d
	/* 2 bytes padding */
	tBTM_ESCO_INFO	esco;			// 0x10
} tSCO_CONN; // size 0x34

typedef struct
{
	tBTM_SCO_IND_CBACK	*app_sco_ind_cb;			// 0x00
	tSCO_CONN			sco_db[BTM_MAX_SCO_LINKS];	// 0x04
	BD_ADDR				xfer_addr;					// 0xa0
	UINT16				sco_disc_reason;			// 0xa6
	tBTM_ESCO_PARAMS	def_esco_parms;				// 0xa8
	BOOLEAN				esco_supported;				// 0xb8
	tBTM_SCO_TYPE		desired_sco_mode;			// 0xb9
	tBTM_SCO_TYPE		xfer_sco_type;				// 0xba
	/* 1 byte padding */
} tSCO_CB; // size 0xbc
...
typedef struct
{
	...
	UINT8	orig_service_name[BTM_SEC_SERVICE_NAME_LEN + 1];	// 0x10
	UINT8	term_service_name[BTM_SEC_SERVICE_NAME_LEN + 1];	// 0x26
} tBTM_SEC_SERV_REC; // size 0x3c

typedef struct
{
	tBTM_SEC_SERV_REC	*p_cur_service;								// 0x00
	tBTM_SEC_CALLBACK	*p_callback;								// 0x04
	void				*p_ref_data;								// 0x08
	UINT32				timestamp;									// 0x0c
	UINT32				trusted_mask[BTM_SEC_SERVICE_ARRAY_SIZE];	// 0x10
	UINT16				hci_handle;									// 0x18
	UINT16				clock_offset;								// 0x1a
	BD_ADDR				bd_addr;									// 0x1c
	DEV_CLASS			dev_class;									// 0x22
	LINK_KEY			link_key;									// 0x25
	tBTM_BD_NAME		sec_bd_name;								// 0x35
	UINT8				sec_flags;									// 0x76
	BD_FEATURES			features;									// 0x77
	UINT8				sec_state;									// 0x7f
	BOOLEAN				is_originator;								// 0x80
	UINT8				security_required;							// 0x81
	BOOLEAN				link_key_not_sent;							// 0x82
	UINT8				link_key_type;								// 0x83
	BOOLEAN				link_key_changed;							// 0x84
	BOOLEAN				sec_acl_connected;							// 0x85
	/* 2 bytes padding */
} tBTM_SEC_DEV_REC; // size 0x88

typedef struct
{
	tBTM_LOC_BD_NAME	bd_name;			// 0x00
	BOOLEAN				pin_type;			// 0x20
	UINT8				pin_code_len;		// 0x21
	PIN_CODE			pin_code;			// 0x22
	BOOLEAN				connectable;		// 0x32
	UINT8				def_inq_scan_mode;	// 0x33
} tBTM_CFG; // size 0x34

typedef struct
{
	tBTM_PM_PWR_MD	req_mode[BTM_MAX_PM_RECORDS];	// 0x00
	tBTM_PM_PWR_MD	set_mode;						// 0x14
	UINT16			interval;						// 0x1e
	UINT8			state;							// 0x20
	UINT8			chg_ind;						// 0x21
...
typedef struct
{
	tBTM_PM_STATUS_CBACK	*cback;	// 0x00
	UINT8					mask;	// 0x04
...
typedef struct
{
	TIMER_LIST_ENT		disc_timer_ent;						// 0x0000
	UINT16				num_uuid_filters;					// 0x0018
	/* 2 bytes padding */
	tBT_UUID			uuid_filters[SDP_MAX_UUID_FILTERS];	// 0x001c
	UINT16				num_attr_filters;					// 0x0058
	UINT16				attr_filters[SDP_MAX_ATTR_FILTERS];	// 0x005a
	/* 2 bytes padding */
	tBTM_INQ_INFO		*p_cur_ii;							// 0x0074
	tBTM_CMPL_CB		*p_disc_cmpl_cb;					// 0x0078 // TODO: is this actually something else? (probably not?)
	UINT8				btm_db_area[BTM_DISC_DB_SIZE];		// 0x007c
	UINT8				disc_active;						// 0x101c // tBTM_DISC_MODE
	/* 3 bytes padding */
...
{
	tBTM_CFG				cfg;													// 0x0000
	tACL_CONN				acl_db[MAX_L2CAP_LINKS];								// 0x0034
	UINT8					btm_scn[BTM_MAX_SCN];									// 0x04a4
	/* 1 byte padding */
	UINT16					btm_def_link_policy;									// 0x04c4
	UINT16					btm_def_link_super_tout;								// 0x04c6
	tBTM_ACL_DB_CHANGE_CB	*p_acl_changed_cb;										// 0x04c8
	tBTM_PM_MCB				pm_mode_db[MAX_L2CAP_LINKS];							// 0x04cc
	tBTM_PM_RCB				pm_reg_db[BTM_MAX_PM_RECORDS];							// 0x0554
	UINT8					pm_pend_link;											// 0x0564
	UINT8					pm_pend_id;												// 0x0565
	/* 2 bytes padding */
	tBTM_DEVCB				devcb;													// 0x0568
	UINT16					btm_acl_pkt_types_supported;							// 0x0654
	UINT16					btm_sco_pkt_types_supported;							// 0x0656
	tBTM_DISCOVERY_VAR_ST	btm_disc_vars;											// 0x0658
	tBTM_DISCOVERY_DB		*p_btm_ddb;												// 0x1678
	tBTM_INQUIRY_VAR_ST		btm_inq_vars;											// 0x167c
	tBTM_FILTER_CB			*p_inq_filter_cb;										// 0x184c
	tSCO_CB					sco_cb;													// 0x1850
	tBTM_APPL_INFO			api;													// 0x190c
	tBTM_LINK_KEY_CALLBACK	*p_link_key_callback[BTM_SEC_MAX_LINK_KEY_CALLBACKS];	// 0x1924
	tBTM_RMT_NAME_CALLBACK	*p_rmt_name_callback[BTM_SEC_MAX_RMT_NAME_CALLBACKS];	// 0x192c
	tBTM_FILTER_CB			*p_conn_filter_cb;										// 0x1934
	tBTM_SEC_DEV_REC		*p_collided_dev_rec;									// 0x1938
	TIMER_LIST_ENT			sec_pin_code_req_tle;									// 0x193c
	BD_ADDR					sec_pin_code_req_bd_addr;								// 0x1954
	/* 2 bytes padding */
	TIMER_LIST_ENT			sec_collision_tle;										// 0x195c
	UINT32					dev_rec_count;											// 0x1974
	UINT8					security_mode;											// 0x1978
	BOOLEAN					pairing_disabled;										// 0x1979
	BOOLEAN					connect_only_paired;									// 0x197a
	BOOLEAN					security_mode_changed;									// 0x197b
	BOOLEAN					pin_type_changed;										// 0x197c
	BOOLEAN					bond_created_connection;								// 0x197d
	UINT8					disconnect_after_bond_reason;							// 0x197e
	UINT8					pin_code_len;											// 0x197f
	PIN_CODE				pin_code;												// 0x1980
	tBTM_SEC_SERV_REC		sec_serv_rec[BTM_SEC_MAX_SERVICE_RECORDS];				// 0x1990
	tBTM_SEC_DEV_REC		sec_dev_rec[BTM_SEC_MAX_DEVICE_RECORDS];				// 0x1f90
	tBTM_MKEY_CALLBACK		*mkey_cback;											// 0x27b0
	BD_ADDR					connecting_bda;											// 0x27b4
	DEV_CLASS				connecting_dc;											// 0x27ba
	tBTM_AFH_CHANNEL		first_disabled_channel;									// 0x27bd
	tBTM_AFH_CHANNEL		last_disabled_channel;									// 0x27be
	UINT8					acl_disc_reason;										// 0x27bf
	tBT_TRACE_LEVEL			trace_level;											// 0x27c0
	/* 3 bytes padding */
} tBTM_CB; // size 0x27c4
