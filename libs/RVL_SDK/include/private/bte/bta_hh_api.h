...
enum
{
	BTA_HH_VIRTUAL_CABLE = 0x0001,
	BTA_HH_NORMALLY_CONNECTABLE = 0x0002,
	BTA_HH_RECONN_INIT = 0x0004,
	BTA_HH_SDP_DISABLE = 0x0008,
	BTA_HH_BATTERY_POWER = 0x0010,
	BTA_HH_REMOTE_WAKE = 0x0020,
	BTA_HH_SUP_TOUT_AVLBL = 0x0040,
	BTA_HH_SEC_REQUIRED = 0x8000
};
...
typedef struct
{
	tBTA_HH_DEVT	tod;	// 0x00
	UINT8			app_id;	// 0x01
} tBTA_HH_SPT_TOD; // size 0x02

typedef struct
{
	UINT8			max_devt_spt;	// 0x00
	/* 3 bytes padding */
	tBTA_HH_SPT_TOD	*p_devt_list;	// 0x04
	UINT16			sdp_db_size;	// 0x08
	/* 2 bytes padding */
...
typedef struct
{
	UINT8	this_char;	// 0x00
	BOOLEAN	caps_lock;	// 0x01
	BOOLEAN	upper;		// 0x02
	BOOLEAN	ctrl;		// 0x03
	BOOLEAN	alt;		// 0x04
	BOOLEAN	num_lock;	// 0x05
} tBTA_HH_KEYBD_RPT; // size 0x06

typedef struct
{
	UINT8	mouse_button;	// 0x00
	INT8	delta_x;		// 0x01
	INT8	delta_y;		// 0x01
...
typedef struct
{
	tBTA_HH_BOOT_RPT_ID	dev_type;	// 0x00
	union
	{
