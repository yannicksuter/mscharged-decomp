...
typedef struct
{
	UINT8	hci_version;	// 0x00
	/* 1 byte padding */
	UINT16	hci_revision;	// 0x02
	UINT8	lmp_version;	// 0x04
	/* 1 byte padding */
	UINT16	manufacturer;	// 0x06
	UINT16	lmp_subversion;	// 0x08
} tBTM_VERSION_INFO; // size 0x0a

typedef struct
{
	UINT16	opcode;							// 0x00
	UINT16	param_len;						// 0x02
	UINT8	param_buf[HCI_COMMAND_SIZE];	// 0x04
	/* 1 byte padding */
...
typedef struct
{
	tBTM_INQ_TYPE			mode;				// 0x00
	UINT8					duration;			// 0x01
	UINT8					max_resps;			// 0x02
	tBTM_INQ_FILT_COND_TYPE	filter_cond_type;	// 0x03
	tBTM_INQ_FILT_COND		filter_cond;		// 0x04
...
typedef struct
{
	tBTM_INQ_RESULTS	results;				// 0x00
	BOOLEAN				appl_knows_rem_name;	// 0x10
	/* 1 byte padding */
} tBTM_INQ_INFO; // size 0x12

typedef struct
{
	tBTM_STATUS	status;		// 0x00
	UINT8		num_resp;	// 0x01
...
typedef struct
{
	UINT16	status;			// 0x00
	UINT16	length;			// 0x02
	BD_NAME	remote_bd_name;	// 0x04
} tBTM_REMOTE_DEV_NAME; // size 0xfc

typedef struct
{
	tBTM_STATUS	status;		// 0x00
	tHCI_STATUS	hci_status;	// 0x01
	BD_ADDR		rem_bda;	// 0x02
	UINT16		settings;	// 0x08
} tBTM_LINK_POLICY_RESULTS; // size 0x0a

typedef struct
{
	tHCI_STATUS	hci_status;		// 0x00
	tBTM_ROLE	role;			// 0x01
	BD_ADDR		remote_bd_addr;	// 0x02
...
{
	tBTM_STATUS	status;		// 0x00
	UINT8		hci_status;	// 0x01
	INT8		rssi;		// 0x02
	BD_ADDR		rem_bda;	// 0x03
...
typedef struct
{
	UINT32	tx_bw;			// 0x00
	UINT32	rx_bw;			// 0x04
	UINT16	max_latency;	// 0x08
	UINT16	voice_contfmt;	// 0x0a
	UINT16	packet_types;	// 0x0c
	UINT8	retrans_effort;	// 0x0e
	/* 1 byte padding */
} tBTM_ESCO_PARAMS; // size 0x10

typedef struct
{
	UINT16	max_latency;	// 0x00
	UINT16	packet_types;	// 0x02
	UINT8	retrans_effort;	// 0x04
	/* 1 byte padding */
} tBTM_CHG_ESCO_PARAMS; // size 0x06

typedef struct
{
	UINT16	rx_pkt_len;		// 0x00
	UINT16	tx_pkt_len;		// 0x02
	BD_ADDR	bd_addr;		// 0x04
	UINT8	link_type;		// 0x0a
	UINT8	tx_interval;	// 0x0b
	UINT8	retrans_window;	// 0x0c
	UINT8	air_mode;		// 0x0d
...
{
	UINT16	sco_inx;		// 0x00
	UINT16	rx_pkt_len;		// 0x02
	UINT16	tx_pkt_len;		// 0x03
	BD_ADDR	bd_addr;		// 0x06
	UINT8	hci_status;		// 0x0c
	UINT8	tx_interval;	// 0x0d
	UINT8	retrans_window;	// 0x0e
	/* 1 byte padding */
...
typedef struct
{
	tBTM_AUTHORIZE_CALLBACK		*p_authorize_callback;		// 0x00
	tBTM_PIN_CALLBACK			*p_pin_callback;			// 0x04
	tBTM_LINK_KEY_CALLBACK		*p_link_key_callback;		// 0x08
	tBTM_LINK_KEY_REQ_CALLBACK	*p_link_key_req_callback;	// 0x0c
	tBTM_AUTH_COMPLETE_CALLBACK	*p_auth_complete_callback;	// 0x10
	tBTM_ABORT_CALLBACK			*p_abort_callback;			// 0x14
...
typedef struct
{
	UINT8	event;		// 0x00
	UINT8	status;		// 0x01
	UINT16	max_keys;	// 0x02
	UINT16	read_keys;	// 0x04
