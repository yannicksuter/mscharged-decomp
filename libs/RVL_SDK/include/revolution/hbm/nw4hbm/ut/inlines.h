...
template <typename T> inline T RoundUp(T t, u32 alignment) { return (alignment + t - 1) & ~(alignment - 1); }

template <typename T> inline void* RoundUp(T* pPtr, u32 alignment) {
    u32 value = reinterpret_cast<u32>(pPtr);
    u32 rounded = (alignment + value - 1) & ~(alignment - 1);
    return reinterpret_cast<void*>(rounded);
}

template <typename T> inline T RoundDown(T t, u32 alignment) { return t & ~(alignment - 1); }

template <typename T> inline void* RoundDown(T* pPtr, u32 alignment) {
    u32 value = reinterpret_cast<u32>(pPtr);
