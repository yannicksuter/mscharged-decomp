...
| Revolution DWC 1.4.1 (2007-05-11) | Exact-date Final and Debug account, encoding-session, ranking-session, and GameSpy socket objects, public account/non-port headers, build flags, source paths, symbols, signatures, structure layouts, and C89 local-variable metadata used to restore account source structure, the separate `dwc_encsession.c`/`dwc_ranksession.c` ownership boundary, and the Revolution `getlocalhost` implementation. |
